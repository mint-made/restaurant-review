...
export class GoogleMap {
    ...
    updateMarkersMapSidebar(map, data, pos, filter = this.filter) {
        console.log("update: ", this.userSubmittedData, this.data)
        this.pos = pos;
        map.setCenter(pos);
    ...
    initLocationSearch(map) {
        ...
        document.querySelector("#search-btn").addEventListener("click", () => {
            ...
            this.placesAPIQueryRequest(query)
                .then(result => {
                    const pos = {
                        lat: result[0].geometry.location.lat(),
                        lng: result[0].geometry.location.lng()
                    }
                    this.placesAPINearbyRequest(pos)
                        .then(results => {
                            this.data = this.modifyPlacesNearbyDataFormat(results);
                            console.log("location search by user - places query returned:", pos, "with Places Nearby Request made giving this data:", this.data);
    ...
    initAddRestaurant(map) {
        document.querySelector("#add-restaurant-btn").addEventListener("click", () => {
            ...
            const sidebarMap = new google.maps.Map(document.querySelector('#add-restaurant-map'), {
                center: this.pos,
                zoom: 14,
                mapTypeControl: false,
                streetViewControl: false,
                fullscreenControl: false,
                draggableCursor: 'url(http://maps.gstatic.com/mapfiles/ridefinder-images/mm_20_purple.png), auto',
                styles: styles
            });
    ...
    initGeolocation(map) {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(position => {
                ...
                this.placesAPINearbyRequest(this.pos)
                    .then(results => {
                        this.data = this.modifyPlacesNearbyDataFormat(results);
                        console.log("New locaton from Geolocation:", this.pos, "So a Places Nearby Request was made:", this.data);
    ...
    initFilter(map) {
        ...
        min.forEach(star => {
            star.addEventListener("click", () => {
                event.preventDefault();
                if (Number.parseInt(event.currentTarget.attributes.value.nodeValue) > this.filter.max) {
                    console.log("*Incorrect input*: User min value > max value");
                    return;
                }
                this.filter.min = Number.parseInt(event.currentTarget.attributes.value.nodeValue);
                for (let i = 0; i < 5; i++) {
                ...
                console.log(`User filtered results with min:${this.filter.min} / max:${this.filter.max} rating`);
                this.updateMarkersMapSidebar(map, this.data, this.pos);
            })
        })
        max.forEach(star => {
            star.addEventListener("click", () => {
                event.preventDefault();
                if (Number.parseInt(event.currentTarget.attributes.value.nodeValue) < this.filter.min) {
                    console.log("*Incorrect input*: User max value < min value");
                    return;
                }
                this.filter.max = Number.parseInt(event.currentTarget.attributes.value.nodeValue);
                for (let i = 0; i < 5; i++) {
                ...
                console.log(`User filtered with min:${this.filter.min} / max:${this.filter.max} rating`);
                this.updateMarkersMapSidebar(map, this.data, this.pos);
            })
        })
    }
    ...
    initLegend(map) {
        ...
        map.controls[google.maps.ControlPosition.LEFT_TOP].push(document.querySelector("#legend"));
    }
    ...
    filterResults(map, data, filter = this.filter) {
        ...
        this.filteredData = this.filteredData.filter(item => item.avgRating <= filter.max);
    }
    ...
    getReviewInfoBox(restaurant) {
        ...
        if (restaurant.ratings.length > 0) {
            ...
            this.placesAPIDetailsRequest(restaurant.placeId)
                .then(results => {
                    this.modifyPlacesDetailsDataFormat(results, restaurant);
                    console.log("No reviews in data, so a Places API Request was made", restaurant, this.data);
    ...
    btnReviewsListener(data = this.data) {
        ...
        btnReviews.forEach(button => {
            button.addEventListener("click", () => {
                const restaurant = data.find(item => item.id === event.currentTarget.id);
                this.getReviewInfoBox(restaurant);
            })
        })
    }
    newRestaurantListener(map) {
        ...
        document.querySelector("#btn-submit-restaurant").addEventListener("click", () => {
            event.preventDefault();
            ...
            console.log("New Restaurant added with the following data:", newRestaurantData);
    ...
    placesAPIDetailsRequest(placesId) {
        const request = {
            placeId: placesId,
            fields: ['name', 'rating', 'website', 'price_level', 'review', 'user_ratings_total',]
        }
        return new Promise((resolve, reject) => {
